...
- **`onVar` の中継が engine 側のダイアログ実装に無い**。 `WantsVarNotify()` は
  「TJS 側に `onVar` があるか」で見るが、 `ElementsScreenDialogImpl` のような
  engine 側のダイアログ実装には `onVar` が無いので、 ホストは
  **インスタンスへ直に生やす**ことになる (TJS はクラスへメソッドを足しても
  既にあるインスタンスには届かないため、 派生クラス側で定義しても間に合わない
  ケースがある)。 いまホスト側はこう書いている:

  ```tjs
  dlg.sysScreenOwner = this;
  dlg.onVar = function(name, value) { ... } incontextof dlg;
  dlg.watchVars = varWatchList;
  ```

  engine 側のダイアログ実装に **`onVar` → owner への中継**を置いておけば、
  ホストは「ドライバに `onVar` を書く」だけで済む
- **`watchVars` の既定値の意味**がドキュメントから読み取れない
  (`"*"` / 名前配列 / `[]` は書いてあるが `void` = 既定が何をするか)。
  実装は「`onVar` があるときだけ全変数」なので、 そう書いてあると迷わない
...
## 9. 同じチャンネルの `animate` が連ならない (優先度: 中。 未着手)
