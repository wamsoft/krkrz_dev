...
- **スクロールバーは `atlas_scrollbar`**: `"thumb"`(9-slice 可)+ `"track"`(省略可)を `index_offset_var`(+`count_var`/`visible`)か `value_var` へ直結。つまみ長は「見えている行数 ÷ 総件数」に比例し、**溝クリックでページ送り・ホイール・ドラッグ**まで内蔵。`list` と同じ `index_offset_var` を挿すだけで連動する。
