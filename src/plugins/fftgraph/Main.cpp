...
#include "tp_stub.h"
#include <math.h>
#include <stdint.h>  // intptr_t (x64 でのポインタ受け渡し)
//---------------------------------------------------------------------------
	...
	tjs_int channels, tjs_int aheadsamples)
{
	...
	// [x64] ポインタを tjs_int (32bit) で受け渡すと上位が落ちる。intptr_t 経由にする
	val[0] = (tjs_int64)(intptr_t)SampleBuffer;
...
class tDrawFFTGraphFunction : public tTJSDispatch
{
		...
		tjs_int numparams, tTJSVariant **param, iTJSDispatch2 *objthis)
	{
		...
		// [x64] 同上。tjs_int でポインタを受けない
		bufferptr = (tjs_uint8 *)(intptr_t)(tjs_int64)val;
...
// エクスポートの書き方は src/plugins の他プラグイン (fpslimit 等) に合わせる。
// もとは Borland の `_stdcall _export` + fftgraph.def だった。
