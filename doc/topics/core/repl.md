...
**既定は本体の言語設定に従います** ( `-language=` や OS 言語がそのまま反映され、
エンジンのメッセージと同じ言語になります )。上部の言語セレクタでその場で
切り替えることもでき、選択はそのブラウザに記憶されます。

現在の対応は **日本語 / 英語**。本体が別の言語 ( 中国語等 ) を選んでいる場合は、
エンジンと同じ落とし方で英語へフォールバックします。
