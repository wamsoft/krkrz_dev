...
⚠ **同じ物理ボタンに両系統を割り当てると 1 押しで 2 回発火します。** 両方の
キーイベントが届く仕様なので、例えば「決定」に `VK_PAD1` ( 刻印 A ) と
`VK_PAD_FACE_SOUTH` ( 下のボタン ) を両方割り当てると、Xbox 系コントローラでは
どちらも同じ物理ボタンを指すため 2 回実行されます。ボタンごとにどちらか一方に
揃えてください。Elements の画面 JSON で両系統を併用していると、画面の構築時に
注意ログが出ます。
...
指定すると、この判定に基づいてボタン絵テーマが自動選択されます。判定はパッドの
接続数と系統を見張っていて変化したその場で決め直されるため、コントローラを抜き
差ししても画面を開き直す必要はありません (表示中の画面のボタン絵も次の描画で
差し替わります)。系統が判定できないプラットフォームでは、パッドが 1 つでも
