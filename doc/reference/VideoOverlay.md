...
- [numberOfVideoStream](#numberofvideostream)
...
- [onCallbackCommand](#oncallbackcommand)
...
現在再生に使用しているビデオストリームのインデックスを取得 / 設定します。
設定すると `SelectVideoStream` が呼ばれ、対応するストリームに切り替わります。

**関連:** [VideoOverlay.numberOfVideoStream](VideoOverlay.md#numberofvideostream)
...
開いている動画に含まれるビデオストリームの本数を返します。読み出し専用。

**関連:** [VideoOverlay.enabledVideoStream](VideoOverlay.md#enabledvideostream)
...
| --- | --- | --- |
| `command` | `&nbsp;` | コマンド名を指定します。 |
| `arg` | `&nbsp;` | コマンドに付随する引数を指定します。 |

**解説**

カスタムコマンドコールバック ( イベント )

動画プレーヤーから任意のコマンド通知を受け取った際に発火するイベントです。
