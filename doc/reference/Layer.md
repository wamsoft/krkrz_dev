...
画像バッファ ( メイン画像と領域画像 ) に加えて、親子ツリー・表示位置・不透明度・入力イベントを持つ
「表示される画像」です。表示を伴わない作業用の画像は [Bitmap](Bitmap.md) クラスで扱い、その加工には
[ImageFunction](ImageFunction.md) クラスを使います。
...
このレイヤノードがフォーカスを持つことが可能かどうかを返します。
親階層が含む `focusable` を考慮した結果を反映します。読み出し専用。

**関連:** [Layer.focusable](Layer.md#focusable)
...
| `text` | `&nbsp;` | 描画する文字を指定します。 |
| `color` | `&nbsp;` | 描画する文字の色を 0xRRGGBB 形式 ( 24bit ) で指定します。<br>**注意:** 上位 8bit (アルファ) は 0 にしてください。上位 8bit が 0 以外の<br>値 ( 例: 白のつもりで 0xFFFFFFFF ) を指定すると、その値は「システムカラー<br>番号」として解釈され ( 内部で `TVPToActualColor` を経由 )、意図しない色<br>( 多くの場合ほぼ黒 ) になります。不透明度は color ではなく opa 引数で<br>指定してください。<br>( [Layer.fillRect](Layer.md#fillrect) / [Layer.colorRect](Layer.md#colorrect) /<br>[Layer.operateRect](Layer.md#operaterect) などの色は 0xAARRGGBB 形式で<br>アルファを含みますが、drawText / drawGlyph などフォント描画系の color は<br>24bit RGB である点が異なります。) |
...
| `y` | `&nbsp;` | 文字描画を開始する原点の ( 画像位置における ) y 座標をピクセル単位で指定します。 |
| `glyph` | `&nbsp;` | 描画するグリフを指定します。 |
| `color` | `&nbsp;` | 描画する文字の色を 0xRRGGBB 形式 ( 24bit ) で指定します。<br>**注意:** [Layer.drawText](Layer.md#drawtext) の color と同様、上位 8bit は<br>0 にしてください ( 0xFFFFFFFF 等を渡すとシステムカラー扱いになり意図しない<br>色になります )。不透明度は opa 引数で指定します。 |
...
| 引数 | 既定値 | 説明 |
| --- | --- | --- |
| `rweight` | `&nbsp;` | 赤成分の重みを指定します。3 つとも省略するか 3 つとも指定してください。 |
| `gweight` | `&nbsp;` | 緑成分の重みを指定します。 |
| `bweight` | `&nbsp;` | 青成分の重みを指定します。 |
...
引数を省略すると BT.709 相当の重み (0.2126 / 0.7152 / 0.0722) を使います
(従来の挙動)。 3 つの重みを渡すと任意の重み付けにできます。 たとえば
BT.601 なら `doGrayScale(0.299, 0.587, 0.114)` です。

重みは相対値として扱い、内部で合計 1 に正規化してから 8 ビット固定小数
(合計 256) へ量子化します。 したがって `0.299, 0.587, 0.114` と
`76, 149, 29` は同じ結果になります。 量子化のため変換結果は理論値から
最大 1 程度ずれます (BT.709 の重みを明示指定した場合は、引数省略時と
完全に同じ結果になります)。

重みが負、または合計が 0 の場合は例外が発生します。
...
| 引数 | 既定値 | 説明 |
| --- | --- | --- |
| `level` | `0` | 出力する階層の深さを指定します。0 を指定するか省略すると<br>配下の全階層を出力します。 |

**解説**

レイヤ構造のダンプ出力

このレイヤを起点としたレイヤツリーの構造をデバッグログへ出力します。
...
| 引数 | 既定値 | 説明 |
| --- | --- | --- |
| `id` | `void` | 解放する個別のタッチ ID を指定します。省略するとこのレイヤが<br>獲得しているすべてのタッチキャプチャを解放します。 |

**解説**

タッチキャプチャの解放

自身が獲得しているタッチキャプチャを解放します。
