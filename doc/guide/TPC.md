...
画像フォーマットコンバータは、TLG画像やPNG画像に「タグ情報」を書き出します。「タグ情報」は、画像どのように表示されるべきかなどを含む情報です。このタグ情報は[Layer.loadImages](Layer.md#loadimages)メソッドの戻り値として得ることができます。
...
- ****vpag_w** **vpag_h** **vpag_unit** (TLG, PNG)**  
  ...
  この情報を PNG に出力するソフトウェアとして [ImageMagick](../guide/http://www.imagemagick.org/.md)ユーティリティがありますが、この **ImageMagick** で trim (トリム)を行った際に、トリム前の画像サイズとしてこの情報が出力されます。
...
- **「透明部分のある画像 - 入力画像をltAddAlpha形式であるとみなす」**  
  このオプションがチェックされていると、入力画像をltAddAlpha形式、つまり吉里吉里の[Layer.type](Layer.md#type)プロパティでltAddAlphaを指定して表示するに適した、加算アルファ合成形式であると見なします。このオプションに影響される入力画像形式はBMP、PNG、メイン/マスク分離形式で、PSD形式は影響されません。
