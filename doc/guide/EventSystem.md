...
吉里吉里では一つのイベントハンドラで必要以上に長い時間がかかることは好ましくありません。時間を待ちたいときは [Timer](../reference/Timer.md) クラスを用いることをおすすめします。
...
非同期イベントをスクリプトの操作で発生させるために [AsyncTrigger](../reference/AsyncTrigger.md) クラスがあります。
...
[Layer](../reference/Layer.md) クラスや [WaveSoundBuffer](../reference/WaveSoundBuffer.md) クラスなどは、コンストラクタに指定したオーナーの action メソッドにイベントを発生する機能があります。

action メソッドには辞書配列オブジェクトの引数が一つ渡され、ここにイベントの情報が入っています。

辞書配列のメンバのうち、 target はイベントの発生元オブジェクトを表しています。type はイベント名を表しています。

その他のメンバはイベントによって異なります。たとえば、[Layer.onClick](../reference/Layer.md#onclick) イベントならば、x と y というメンバがあり、レイヤ上をクリックされた場所を表します。
