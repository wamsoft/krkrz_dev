...
- **-**replwebidle** (ブラウザが閉じたらアプリも終了する秒数)**  
  `-replweb` の SSE 購読が **1 本も無い状態が指定秒数続いたらアプリを終了**します。ブラウザを UI にした構成 ( `-replweb` + アプリモード起動 ) で、ウィンドウを閉じたのに本体だけ残り続けるのを防ぐためのものです。
  
  既定は **0 ( 無効 )**。ブラウザを開かないエージェント駆動や、`-replweb` を単なる API 面として使う構成を巻き込まないためです。また **一度でも購読が来てから武装する**ので、有効にしていてもブラウザを開く前に落ちることはありません。
  
  切断は SSE のハートビート送信が失敗して初めて分かるため、実際の終了は指定秒数 + ハートビート間隔ぶん遅れます ( 武装中はハートビートを指定秒数まで詰めるので、最悪で約 2 倍 )。例: `-replwebidle=5` なら閉じてから 5〜10 秒で終了。
