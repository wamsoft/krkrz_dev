...
プラグインを使用する場合は [Plugins.link](Plugins.md#link) メソッドを使用して吉里吉里本体に接続する必要があります。

## プラグインの検索

[Plugins.link](Plugins.md#link) メソッドでは、指定されたプラグインを以下の順序で探します。
...
- ****wsh.dll** (experimental)**  
  ...
  このプラグインを接続すると、[Scripts.exec](Scripts.md#exec) と [Scripts.execStorage](Scripts.md#execstorage) が拡張されます。
