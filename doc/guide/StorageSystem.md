...
[Storages.getFullPath](Storages.md#getfullpath) メソッドは、OS の使うファイル名から統一ストレージ名への変換を行うことができます。この過程で、ファイル名中の大文字はすべて小文字に変換されます ( ローカルストレージについては大文字と小文字は区別されません )。また、パス区切りに \ が用いられている場合は / に変換されます。

[Storages.getLocalName](Storages.md#getlocalname) メソッドは、統一ストレージ名から OS の使うファイル名への変換を行うことができます。吉里吉里以外のアプリケーションにストレージ名を渡す場合などはこのメソッドを使用する必要があります。
...
の -datapath オプションで指定されたフォルダです。[System.dataPath](System.md#datapath) プロパティで取得することができます。
