...
split メソッドは、対象文字列を正規表現パターンにマッチする箇所で分割します。
...
パターンにキャプチャグループ ( `(...)` ) が含まれる場合、各マッチ区間の文字列に続けて、そのマッチでキャプチャされた部分文字列も順に配列へ格納されます ( JavaScript の String.split や 吉里吉里2 と同じ挙動 )。マッチしなかったグループは空文字列として格納されます。

第 3 引数に true を指定すると、空の要素 ( 区間・キャプチャグループの双方 ) を結果から取り除きます ( 既定は false )。

String クラスの split メソッドも参照してください。
