...
| 中 | ✅ バリアブルフォント (可変軸) の TJS 露出 (P0〜P4) | **実装済** (src/core b48ba3bc, 2026-08-23)。`Font.weight` / `Font.variations` / `Font.defaultUseVarStyle` / `Font.getVarAxes` / `getFontInfo` の axes・namedInstances / `fonts.json` の `axes`・`instance` 宣言。glyphware 経路 (`rasterizer=2` の drawText + drawShapedText 系) のみ。**残 = P5 (Elements / ThorVG gw ブリッジへの軸伝搬)**。SSOT = [FontEngine.md](src/core/doc/FontEngine.md) 「バリアブルフォント (可変軸) の全体展開」 |
