...
| 【検討中】 | Elements: リピート «処理» そのものの統合 | §6 で揃えたのは設定値だけで、リピートを作る場所は 3 系統 (キーボード=OS / 十字・スティック=elements のタイマ / VK_PAD*=engine) のまま。**案 A (エンジンが全部作る) はそのままでは成立しない** — engine のリピート生成器はボタン状態からしか作らずスティックが対象外で、寄せると「十字=engine クロック / スティック=elements クロック」と今より不統一になる。**案 B (elements が全部持つ)** は 1 クロックにできるが、キーアップ取りこぼしでフォーカスが走る事故の範囲をキーボードへ広げる (§5 のガードが守っている領域)。**案 C (推奨) = スロットル**: ブリッジが `on_key_down` に repeat フラグを渡し (elements 側には `key_action::repeat` が元からある)、view がナビキーの repeat に `axis_repeat` と同じ delay/rate を当てて早すぎる分を**捨てる**。生成せず落とすだけなので暴走経路が増えず held 状態も不要。制約は「OS より遅くはできるが速くはできない」。規模はブリッジ 1 引数 + view 30 行程度。**完全統合するならエンジンが権威で正しい**が、(1) elements 側の機構は消さず «ホストが駆動するモード» として切れるようにする (krkrz 非依存の汎用ライブラリなので)、(2) エンジンが持てるのはリピートのクロックまでで、スティックの value モード (倒し量連動 / デッドゾーン) は生の軸値が要るので elements に残る、の 2 条件付き。着手するなら §5 のガード整理の後が安全。詳細 = [ElementsAudit.md](src/core/doc/ElementsAudit.md) §6 |
