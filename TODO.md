...
| 高 | Elements: 一覧の更新コストが行数に比例する | 1 枚のパネル内でラベル 1 個だけを `setVar` しても、ラスタ 1 回のコストが list の行数に比例する (8 行 413us → 64 行 1059us)。**2026-09-12 に真因を特定**: `list_rows_element::limits()` が毎フレーム呼ばれ、その中で全行の `limits()` を回している (rows=64 で 60 フレームあたり 3840 回。行はラベルなので毎回テキスト計測が走る)。`list::draw` のカリングは効いていて部分再描画中はそもそも呼ばれないのに、この経路だけ O(行数) で残る。コメントの「副作用のある widget 用」= `text_var` に新しい値を拾わせる目的なので、変数変化の通知で代替できるはず。先に「なぜ `limits()` が毎フレーム呼ばれるのか (view の毎フレームレイアウト検証)」を確かめるのが本筋。計測 = `data/elements_audit -audittest`。詳細 = [ElementsAudit.md](src/core/doc/ElementsAudit.md) §2 |
