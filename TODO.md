...
| 中 | Steam Deck でオンスクリーンキーボードが意図せず表示される | gamescope/Steam はアプリが SDL text input を有効にしているとソフトキーボードを出す。Steam Deck 実機 (devkit 実行、SDL3 Linux ビルド) で起動直後から OSK が出ることを確認 (2026-08-24)。調査起点 = `src/core/sdl3/environ/form.cpp` の `TVPUpdateBaselineTextInput`: `SDL_HasScreenKeyboardSupport()` が Deck 環境で false (または `SDL_HasKeyboard()` true) と判定され常時 `SDL_StartTextInput` になる疑い。Elements 側 (`ElementsDialogManager.cpp` の `PlatformUsesScreenKeyboard` / `UpdateFocusDrivenTextInput`) には既にフォーカス駆動ゲートがあるので、Deck をスクリーンキーボード環境として検出しフォーカス駆動へ載せるのが修正方針候補。gamescope 検出は環境変数 (`SteamDeck=1` / `XDG_CURRENT_DESKTOP=gamescope`) 等が使える |
