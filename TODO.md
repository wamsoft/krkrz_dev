...
1. **ホストの描画中に文字だけ出ない** — 絵は出るのに文字が全部消えるので、
   ゲーム中の環境設定・名前入力の画面がそのまま使えない。原因未特定
...
| 優先 | 課題 | 内容 |
|---|---|---|
| 高 | Elements: ホストの描画中に文字だけ出ない | ホスト案件で、シーン再生中に開いた overlay は `label` / `button` / `input_box` の**文字だけ**描かれない (絵は出る)。 同じ画面をタイトルから開くと出る。 GL 描画 off / フォント再登録 / レイヤ非表示 のいずれも効かず、当たりは**テキスト描画のバックエンド (glyphware / ThorVG 経路) がホストの描画中だけ使えなくなっている**線。 詳細 = [TODO-elements.md](TODO-elements.md) §6 |
