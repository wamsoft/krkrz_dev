...
| 中 | バリアブルフォント (可変軸) の TJS 露出 | エンジン (glyphware) と tp_stub は **対応済み** (`TVPFontGetVarAxes` / `TVPFontSetVariations` / `TVPFontAcquireFaceInstance`)。欠けているのは TJS のフォントパラメータとしての指定口。**方針決定済 = ①glyphware ラスタライザ (`rasterizer=2`) 経路のみ (旧 FreeType / GDI へは実装せず、将来 glyphware へ一本化) ②`font.weight` (一級) + `font.variations` (汎用軸) ③bold/italic の軸マッピングは既定 OFF のオプトイン ④軸値の量子化 + 軸付き face の LRU ⑤`fonts.json` は `axes` / named `instance` の両対応 ⑥フォールバック連鎖は同名軸のみ適用**。 P0 (glyphware に namedInstances) → P1 (`tTVPFont` 拡張・本丸) → P2 (fonts.json) → P3 (照会 API) → P4 (自動マッピング) → P5 (Elements/gw)。SSOT = [FontEngine.md](src/core/doc/FontEngine.md) 「バリアブルフォント (可変軸) の全体展開」 |
