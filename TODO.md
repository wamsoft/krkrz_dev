...
- 起動オプションの残骸 2 件 (受け付けるが何もしない)。 `-ogg_pcm_format` は `common/sound/VorbisCodecDecoder.cpp` でブロックコメントの中にあり、 指定しても効かない (opus 側の `-opus_pcm_format` は生きている)。 `-timerprec` は `generic/base/SysInitImpl.cpp` で値を読んで `prectick` に入れるだけで、 以降どこからも参照されない (WINVER 側にも実装なし)。 どちらもガイドには載せていないので、 **実装を戻すか、 オプションごと消すか**を決めるだけの作業 (2026-09-20 のコマンドラインオプション棚卸しで判明)。
